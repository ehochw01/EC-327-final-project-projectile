...
int main() {
    ...
    bool isNight = false;   // day/night mode, toggled by the day/night button in the bottom-right
    ...
    // day/night toggle, sits just to the left of the mute button (10px gap)
    Rectangle dayNightButton = { (float)screen_width - 200.0f, (float)screen_height - 50.0f, 90.0f, 35.0f };
    ...
    while (!WindowShouldClose()) {    // will be true until we hit escape key
        ...
        BeginDrawing();
            ...
            // day/night toggle button, to the left of mute. label shows the current mode.
            ...
            DrawText(dayNightText,
                     (int)(dayNightButton.x + dayNightButton.width / 2 - dayNightTextWidth / 2),
                     (int)(dayNightButton.y + dayNightButton.height / 2 - dayNightFontSize / 2),
                     dayNightFontSize, BLACK);
